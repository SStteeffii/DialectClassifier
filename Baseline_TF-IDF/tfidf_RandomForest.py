...
def train_random_forest(filename, classifiername, num_samples: int):
...
def prediction_random_forest(classifiername, output_path, filename_out, filename, data_label, pca, tf_idf):
    # load the test data
...
if __name__ == '__main__':
    ...
    output_path = 'Result_RandomForest/mr_dialect/run_5/'
    classifier_name = 'random_forest.joblib'
    ...
    for num_samples in num_samples_list:
        ...
        for file_train in files_train:
            ...
            pca, tf_idf = train_random_forest(file_train, classifier_name, num_samples)

            # load trained classifier, test with validation data
            file_name_val = 'wikipedia_validation'
            prediction_random_forest(classifier_name, output_path_act, file_name_val,
                                     file_validation, 'validate', pca, tf_idf)

            # load trained classifier, test with test data
            file_name_test = 'wikipedia_test'
            prediction_random_forest(classifier_name, output_path_act, file_name_test, file_validation, 'test', pca,
                                     tf_idf)

            # load trained classifier, test with final test data (Tatoeba)
            file_name_final = 'tatoeba'
            prediction_random_forest(classifier_name, output_path_act, file_name_final, file_final_test, 'test', pca,
                                     tf_idf)
