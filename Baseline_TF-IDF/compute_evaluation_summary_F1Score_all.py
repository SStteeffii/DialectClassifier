...
# macro F1-Score for only tatoeba files for each training file - all samples mean with standard deviation
...
if __name__ == '__main__':
    ...
    base_paths = ['./Result_SupportVectorMachine/', './Result_RandomForest/']
    ...
    for base_path in base_paths:
        ...
        # name of the test file, which we are interested in
        test_file = 'wikipedia_test.tsv'

        for train_file in train_files:
            base_path_long = base_path + train_file
            for num in number_samples:
                # the number of samples, which are interesting
                num_samples = num

                # compute the runs
                reports = store_runs(base_path_long, num_samples, test_file)
                ...
                data.append([train_file, num, mean_report['macro'], std_report['macro']])
        ...
        df = pd.DataFrame({
            'Train-Data': [item[0] for item in data],
        ...
        unique_files = df['Train-Data'].unique()

        for file in unique_files:
            file_df = df[df['Train-Data'] == file]
        ...
        plt.xscale('log')
        plt.title('F1-Score Vergleich für Wikipedia Testdatei - ' + base_path[base_path.find("/")+8:base_path.rfind("/")])
        plt.xlabel('Samples')
        ticks = df['Samples']
        ...
        pivot_df = df.pivot(index='Samples', columns='Train-Data', values='F1-Score').reset_index()

        for col in pivot_df.columns[1:]:  # Überspringt die erste Spalte 'Samples'
            pivot_df[col] = (pivot_df[col] * 100).round(2).astype(str) + '%'
        pivot_df = pivot_df.round(2)
        pivot_df['Samples'] = pivot_df['Samples'].astype(int)
        ...
        plt.savefig(str(base_path[:base_path.rfind('/')] + '/' + base_path[2:base_path.rfind('/')] + '_test_wikipedia_plot.png'), dpi=300)
        plt.show()
